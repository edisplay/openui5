sap.ui.define([
	...
	return ManagedObject.extend("mdc.sample.model.mockServer", {
		started: null,
		oModel: null,
		server: null,
		init: function () {
		...
		start: function() {
			...
			this.server.respondWith("POST", RegExp("/uploadFiles","i"), function (xhr) {
				...
				const oFile = xhr.requestBody ? xhr.requestBody : null;
				const aRequestArray = [];
				...
				const iExistingDocumentId = requestHeaders.existingDocumentID;
				const data = that.oModel && that.oModel.getData ? that.oModel.getData() : {};
				let oExistingfileObjectData = null;
				...
				const url = requestHeaders && requestHeaders.documentUrl ? requestHeaders.documentUrl : URL.createObjectURL(oFile);
				let fileId = uid(); // generating random id
