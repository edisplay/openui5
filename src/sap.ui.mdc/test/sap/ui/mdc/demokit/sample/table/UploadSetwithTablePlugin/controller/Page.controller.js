sap.ui.define([
	...
	return Controller.extend("mdc.sample.controller.Page", {
		onInit: function () {
			...
			/**
			 * Setting the controller instance to the helper object.
			 * this instance is used in table delegate to bind the controller methods to the column's template.
			 * Please refer to the JSONTableDelegate.js file for more information.
				...
				"Are you sure you want to remove the document" + " " + oContext.getProperty("fileName") + " " + "?",
				{
					...
					onClose: function (sAction) {
						...
						if (spath.split("/")[2]) {
							...
							const data = oModel.getProperty("/items");
							data.splice(index, 1);
							oModel.refresh(true);
		...
		openAddOrEditDialog: function () {
			if (!this._addViaUrlFragment) {
				Fragment.load({
					name: "mdc.sample.view.fragment.AddViaUrl",
					id: "addViaUrlDialog",
					controller: this
				})
