...
["getDateInstance", "getDateTimeInstance", "getTimeInstance"].forEach(function (sMethod) {
	...
	QUnit.test("getSampleValue single date, " + sMethod + "; UTC=" + bUTC, function (assert) {
		const sExpectedDate = (sMethod === "getDateInstance"
			? "2012-12-31T00:00:00.000"
			: "2012-12-31T23:59:58.123") + (bUTC ? "Z" : "");
	...
	QUnit.test("getSampleValue date interval, " + sMethod + "; UTC=" + bUTC, function (assert) {
		const sExpectedEndDate = (sMethod === "getDateInstance"
				? "2012-12-31T00:00:00.000"
				: "2012-12-31T23:59:58.123") + (bUTC ? "Z" : ""),
...

{
	...
	QUnit.test("getSampleValue: R1 - default in range", function (assert) {
		...
		const oExpected = getDate(Date.UTC(2026, 11, 31, 0, 0, 0, 0));
		...
		// code under test - oMaximum exactly at year-end midnight keeps the default year-end in range
		assert.deepEqual(getSample(undefined, getDate("2026-12-31T00:00:00Z")), [oExpected]);
		...
		// code under test - oMinimum is in current year but after Dec 31 00:00:00.000 -> R2 applies
		assert.notDeepEqual(getSample(getDate("2026-12-31T00:00:01Z"), undefined), [oExpected]);
	});

	//*****************************************************************************************************************
	QUnit.test("getSampleValue: R2 - different years - closest year-end", function (assert) {
		const oYearEnd = (iYear) => getDate(Date.UTC(iYear, 11, 31, 0, 0, 0, 0));

		// code under test
		assert.deepEqual(getSample(getDate("2040-01-01T00:00:00Z"), undefined), [oYearEnd(2040)]);
		// code under test
		assert.deepEqual(getSample(getDate("2040-12-31T00:00:01Z"), undefined), [oYearEnd(2041)]);
		// code under test
		assert.deepEqual(getSample(undefined, getDate("2020-06-15T00:00:00Z")), [oYearEnd(2019)]);
		// code under test
		assert.deepEqual(getSample(undefined, getDate("2020-12-31T00:00:00Z")), [oYearEnd(2020)]);
		...
		// code under test - both, different years
		assert.deepEqual(getSample(getDate("2038-06-01T00:00:00Z"), getDate("2040-12-31T00:00:00Z")), [oYearEnd(2040)]);
	});

	//*****************************************************************************************************************
	QUnit.test("getSampleValue: R3 - same year, different months - closest month-end", function (assert) {
		const oMonthEnd = (iYear, iMonth, iDay) => getDate(Date.UTC(iYear, iMonth, iDay, 0, 0, 0, 0));

		// code under test
		assert.deepEqual(getSample(getDate("2040-01-01T00:00:00Z"), getDate("2040-06-15T00:00:00Z")), [oMonthEnd(2040, 4, 31)]);
		// code under test
		assert.deepEqual(getSample(getDate("2040-01-01T00:00:00Z"), getDate("2040-06-30T00:00:00Z")), [oMonthEnd(2040, 5, 30)]);
	});
	...
	QUnit.test("getSampleValue: interval", function (assert) {
		...
		assert.deepEqual(getIntervall(getDate("2040-01-01T00:00:00Z"), getDate("2040-06-30T23:59:59Z")),
			[[getDate(Date.UTC(2040, 5, 21, 9, 12, 34, 567)), getDate(Date.UTC(2040, 5, 30, 0, 0, 0, 0))]]);

		const oMin = getDate("2040-12-25T00:00:00Z");
		// code under test: start clamped to min
		assert.deepEqual(getIntervall(oMin, undefined), [[oMin, getDate(Date.UTC(2040, 11, 31, 0, 0, 0, 0))]]);
