sap.ui.define([
	...
	return Controller.extend("tree.table.sample.controller.Page", {
		onInit: function () {
		...
		onSelectionChange: function(oEvent) {
			...
			if (aSelectedContexts.length === 1 && !bFolderContextSelected?.length) {
		...
		onDownloadFiles: function(oEvent) {
			const aIndices = this.byId("table-uploadSet").getSelectedIndices();
			const aContexts = aIndices.map((iIndex) => this.byId("table-uploadSet").getContextByIndex(iIndex));
			if (aContexts && aContexts.length) {
				aContexts.forEach((oContext) => this.oUploadPluginInstance.download(oContext, true));
			}
		},
		...
		onRemoveButtonPress: function(oEvent) {
			...
			const oContext = oTable.getContextByIndex(aIndices[0]);
		...
		onRenameDocument: function() {
			...
			if (aContexts?.length === 1) {
				...
				this.oUploadPluginInstance.renameItem(aContexts[0]);
			}
		},
		...
		openAddOrEditDialog: function () {
			if (!this._addViaUrlFragment){
				Fragment.load({
					name: "sap.m.sample.UploadSetwithTablePlugin.TreeTableSimple.view.fragment.AddViaUrl",
					id: "addViaUrlDialog",
					controller: this
				})
		...
		onEditUrl: function(oEvent) {
			const oTable = this.byId("table-uploadSet");
			const oIndex = oTable.getSelectedIndices()[0];
			const oBidningContextObject = oTable.getContextByIndex(oIndex)?.getObject(),
			...
			this.oWarningMessageDialog = new Dialog({
				...
				buttons: [ new Button({
					...
					press: [function () {
						...
						iIndex = oTable?.getSelectedIndices()[0];
						const oBidningContextObject = oTable.getContextByIndex(iIndex)?.getObject();
						const oModel = this.getView().getModel("documents");
