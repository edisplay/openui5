...
], function (Controller, UploadSetwithTable, MessageBox, Fragment, MockServer, MessageToast, Dialog, Button, mobileLibrary, Text, coreLibrary, CoreItem, Filter, FilterOperator, Element, JSONModel, URLListValidator)  {
	"use strict";

	return Controller.extend("responsiveFileDialogs.table.sample.controller.Page", {
		onInit: function () {
		...
		openAddOrEditDialog: function () {
			if (!this._addViaUrlFragment){
				Fragment.load({
					name: "sap.m.sample.UploadSetwithTablePlugin.ResponsiveTableFileDialogs.view.fragment.AddViaUrl",
					id: "addViaUrlDialog",
					controller: this
				})
		...
		openFileUploadDialog: function() {
			...
			if (items && items.length) {
				...
				if (!this._fileUploadFragment) {
					Fragment.load({
						name: "sap.m.sample.UploadSetwithTablePlugin.ResponsiveTableFileDialogs.view.fragment.FileUpload",
