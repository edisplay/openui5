sap.ui.define([
	...
	return Controller.extend("fileNameValidationSample.table.sample.controller.Page", {
		onInit: function () {
		...
		openAddOrEditDialog: function () {
			if (!this._addViaUrlFragment){
				Fragment.load({
					name: "sap.m.sample.UploadSetwithTablePlugin.FileNameValidationSample.view.fragment.AddViaUrl",
					id: "addViaUrlDialog",
					controller: this
				})
		...
		onCheckboxSelect: function(oEvent) {
			...
			const cbInclude = this.byId("cbInclude");
			const cbExclude = this.byId("cbExclude");
			const cbOverride = this.byId("cbOverride");

			// Deselect other checkboxes
			cbInclude.setSelected(sId === cbInclude.getId());
			cbExclude.setSelected(sId === cbExclude.getId());
			cbOverride.setSelected(sId === cbOverride.getId());

			let selectedMode = "override";

			if (cbInclude.getId() === sId) {
				selectedMode = "include";
			} else if (cbExclude.getId() === sId) {
			...
			const oModel = this.getView().getModel("documents");
			oModel.setProperty("/mode", selectedMode);
			oModel.refresh(true);
		...
		onInputChange: function (oEvent) {
			...
			// Get the current value from the Input field (it may not be updated in the model yet)
			const characters = oEvent.getParameter("newValue");

			// Set the new value of characters in the model
			oModel.setProperty("/characters", characters);
			oModel.refresh(true);
			this._updateButtonState();
		},
		...
		_updateButtonState: function () {
			const oModel = this.getView().getModel("documents");
			const chars = oModel.getProperty("/characters");
			const mode = oModel.getProperty("/mode");

			const isEnabled = !!chars && !!mode;
			oModel.setProperty("/isConfigureButtonEnabled", isEnabled);
			oModel.refresh(true);
		},
		onConfigureHandler: function (oEvent) {
			var mode = "";
			if (this.byId("cbInclude").getSelected()) {
			  mode = "include";
			} else if (this.byId("cbExclude").getSelected()) {
			  mode = "exclude";
			} else if (this.byId("cbOverride").getSelected()) {
			  mode = "override";
			}

			var characters = this.byId("charInput").getValue();
			...
			this.oUploadPluginInstance.setFileNameValidationConfig(result);

			MessageToast.show(`Configuration mode: ${result.mode}, Characters: ${result.characters}, have been applied to the fileNameValidationConfig.`);
