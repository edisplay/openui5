sap.ui.define([
	...
	return ManagedObject.extend("multiplePluginInstances.table.model.mockServer", {
		started: null,
		oModel: null,
		server: null,
		instancePath: "items",
		...
		_updateModelWithData: function(oResponse, oExistingModelDataToUpdate, sMode, oFile, sUrl, oData) {
			switch (sMode) {
				...
				case "Create":
					this.oModel.getProperty(`/${this.instancePath}`).unshift(
						{
		...
		updateExisitingDocument: function(oFileObject) {
			const items = this.oModel.getProperty(`/${this.instancePath}`);
			...
			if (oExistingfileObjectData) {
				...
				this.oModel.setData({[this.instancePath]: items});
			}
