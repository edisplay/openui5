	...
	Object.keys(oSubLanguages).forEach(function(sLanguage) {
		...
		QUnit.module("Translation mode - language In " + sLanguage + ", out " + sMappingLanguage, {
			...
			Object.keys(oEditorLanguages).forEach(function(sEditorLanguage) {
				QUnit.test("Editor language - " + sEditorLanguage + " : 1 string parameter with Admin, Content and Translation translations", function (assert) {
					...
					return new Promise(function (resolve, reject) {
						EditorQunitUtils.isFieldReady(this.oEditor).then(function () {
							...
							EditorQunitUtils.wait().then(function () {
								var sExpectedValueOfOriText = sLanguage === sEditorLanguage ? "String3 translation" : _oExpectedValuesOfChangesWithTransFormat["string1string2"][sMappingLanguage] || "StringParameter Value Trans in i18n " + sMappingLanguageValue;
