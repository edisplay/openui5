...
sap.ui.define([
	...
	var _sManifestPath = "/sap.card/configuration/parameters/objectsWithPropertiesDefinedAndValueFromJsonList/value";
			...
			QUnit.test("admin change with value 'string1' and no translation texts, content change with value 'string2' and no translation texts", function (assert) {
				...
				return new Promise(function (resolve, reject) {
					EditorQunitUtils.isFieldReady(this.oEditor).then(function () {
						...
						assert.ok(oField.isA("sap.ui.integration.editor.fields.ObjectListField"), "Field: Object List Field");
						EditorQunitUtils.isReady(this.oEditor).then(function () {
							...
							var oSelectedObject = merge(deepClone(oObject2, 500), {"_dt": {"_selected": true, "_position": 1}});
				...
				return new Promise(function (resolve, reject) {
					EditorQunitUtils.isFieldReady(this.oEditor).then(function () {
						...
						EditorQunitUtils.isReady(this.oEditor).then(function () {
							...
							assert.ok(deepEqual(oRow1.getBindingContext().getObject(), oSelectedObject), "Table: value object is the first row");
							var oTextCell1 = oRow1.getCells()[3];
							var sTextPropertyValue = _oExpectedValuesOfChangesWithTransFormat["string1"][sMappingLanguage] || _oExpectedValuesOfChangesWithTransFormat["string2"].default;
							assert.equal(oTextCell1.getText(), sTextPropertyValue, "Row 1: text cell value");
							oTable.setSelectedIndex(0);
							oTable.fireRowSelectionChange({
								rowIndex: 0,
								...
								EditorQunitUtils.wait().then(function () {
									...
									EditorQunitUtils.wait(1500).then(function () {
										...
										for (var i = 0; i < oLanguageItems.length; i++) {
											...
											if (oCustomData && oCustomData.length > 0) {
												var sLanguage = oCustomData[0].getKey();
												var sExpectedValue = _oExpectedValuesOfChangesWithTransFormat["string1"][sLanguage] || _oExpectedValuesOfChangesWithTransFormat["string2"]["default"];
												var sCurrentValue = oLanguageItems[i].getContent()[0].getItems()[1].getValue();
												assert.equal(sCurrentValue, sExpectedValue, "oTranslationPopover Content: item " + i + " " + sLanguage + ", current: " + sCurrentValue + ", expected: " + sExpectedValue);
											}
										}
				...
				return new Promise(function (resolve, reject) {
					EditorQunitUtils.isFieldReady(this.oEditor).then(function () {
						...
						EditorQunitUtils.isReady(this.oEditor).then(function () {
							...
							assert.ok(deepEqual(oRow1.getBindingContext().getObject(), oSelectedObject), "Table: value object is the first row");
							var oTextCell1 = oRow1.getCells()[3];
							var sTextPropertyValue = _oExpectedValuesOfChangesWithTransFormat["string1string2"][sMappingLanguage] || _oExpectedValuesOfChangesWithTransFormat["string2"].default;
							assert.equal(oTextCell1.getText(), sTextPropertyValue, "Row 1: text cell value");
							oTable.setSelectedIndex(0);
							oTable.fireRowSelectionChange({
								rowIndex: 0,
								...
								EditorQunitUtils.wait().then(function () {
									...
									EditorQunitUtils.wait(1500).then(function () {
										...
										for (var i = 0; i < oLanguageItems.length; i++) {
											...
											if (oCustomData && oCustomData.length > 0) {
												var sLanguage = oCustomData[0].getKey();
												var sExpectedValue = _oExpectedValuesOfChangesWithTransFormat["string1string2"][sLanguage] || _oExpectedValuesOfChangesWithTransFormat["string2"]["default"];
												var sCurrentValue = oLanguageItems[i].getContent()[0].getItems()[1].getValue();
												assert.equal(sCurrentValue, sExpectedValue, "oTranslationPopover Content: item " + i + " " + sLanguage + ", current: " + sCurrentValue + ", expected: " + sExpectedValue);
											}
										}
