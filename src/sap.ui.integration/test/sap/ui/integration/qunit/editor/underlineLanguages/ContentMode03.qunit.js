...
sap.ui.define([
	...
	Object.keys(_oLanguageMapping).forEach(function(sLanguage) {
		...
		QUnit.module("Language In " + sLanguage + ", out " + sMappingLanguage, {
			...
			QUnit.test("1 string parameter with Content translations", function (assert) {
				...
				return new Promise(function (resolve, reject) {
					EditorQunitUtils.isFieldReady(this.oEditor).then(function () {
						...
						EditorQunitUtils.isReady(this.oEditor).then(function () {
							...
							var sExpectedValueOfText = _oExpectedValuesOfChangesWithTransFormat["string2"][sMappingLanguage] || "StringParameter Value Trans in i18n " + sMappingLanguageValue;
							...
							oField.attachEventOnce("translationPopoverOpened", function() {
								...
								for (var i = 0; i < oLanguageItems1.length; i++) {
									...
									if (!sExpectedValue) {
										sExpectedValue = _oExpectedValuesOfChangesWithTransFormat["string2"]["default"];
