...
sap.ui.define([
	...
	Object.keys(Utils.languageMapping).forEach(function(sLanguage) {
		...
		QUnit.module("Language In " + sLanguage + ", out " + sMappingLanguage, {
			...
			QUnit.test("1 string parameter with Admin BC change, Translation change", function (assert) {
				...
				this.oEditor.setJson({
					...
					manifestChanges: [oAdminChange, oTranslationChange]
				});
			...
			QUnit.test("1 string parameter with Admin change and BC change, Translation change", function (assert) {
				var oAdminChange = deepClone(_oAdminChangeBase, 500);
