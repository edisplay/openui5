	...
	Object.keys(oSubLanguages).forEach(function(sLanguage) {
		...
		QUnit.module("Translation mode - language In " + sLanguage + ", out " + sMappingLanguage, {
			...
			Object.keys(oEditorLanguages).forEach(function(sEditorLanguage) {
				...
				QUnit.test("Editor language - " + sEditorLanguage + " : 1 string parameter with Content translations", function (assert) {
					...
					return new Promise(function (resolve, reject) {
						EditorQunitUtils.isFieldReady(this.oEditor).then(function () {
							...
							EditorQunitUtils.wait().then(function () {
								var sExpectedValueOfOriText = _oExpectedValuesOfChangesWithTransFormat["string2"][sMappingLanguage] || "StringParameter Value Trans in i18n " + sMappingLanguageValue;
								...
								var sExpectedValueOfTransText = _oExpectedValuesOfChangesWithTransFormat["string2"][sMappingLanguageTrans] || "StringParameter Value Trans in i18n " + sMappingLanguageValueTrans;
