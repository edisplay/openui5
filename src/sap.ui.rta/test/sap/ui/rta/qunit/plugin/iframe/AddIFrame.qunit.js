...
sap.ui.define([
	...
	QUnit.module("Given a designTime and addIFrame plugin are instantiated for an ObjectPageLayout", {
		...
		QUnit.test("when the getMenuItems() is called to check texts, with addIFrame defined on more than one aggregations", async function(assert) {
			...
			assert.strictEqual(
				aMenuItems[0].text, sExpectedSectionText,
			...
			assert.strictEqual(
				aMenuItems[1].text, sExpectedHeaderText,
		...
		QUnit.test("when an overlay has addIFrame action in designTimeMetadata, but has no isEnabled property defined", async function(assert) {
			...
			await DtUtil.waitForSynced(this.oDesignTime)();
			assert.strictEqual(
				this.oAddIFrame.isAvailable([this.oObjectPageLayoutOverlay], {}, false),
				true,
				"then isAvailable is called and it returns true"
			);
			assert.strictEqual(
				this.oAddIFrame.isEnabled([this.oObjectPageLayoutOverlay], { action: oAction }),
				true,
				"then isEnabled is called and it returns true"
			);
			const bIsEditable = await this.oAddIFrame._isEditableCheck(this.oObjectPageLayoutOverlay, false);
		...
		QUnit.test("when an overlay has addIFrame action designTime metadata, has no changeType and isEnabled property is true", async function(assert) {
			...
			assert.strictEqual(
				this.oAddIFrame.isAvailable([this.oObjectPageLayoutOverlay], {}, false),
				false,
				"then isAvailable is called and then it returns false"
			);
			assert.strictEqual(
				this.oAddIFrame.isEnabled([this.oObjectPageLayoutOverlay], { action: oAction }),
				true,
				"then isEnabled is called and then it returns correct value"
			);
			const bIsEditable = await this.oAddIFrame._isEditableCheck(this.oObjectPageLayoutOverlay, false);
			assert.notOk(bIsEditable, "then the overlay is not editable");
		});
		...
		QUnit.test("when an overlay has addIFrame action on a responsible element", async function(assert) {
			...
			async function checkMenuItemWhenActionAvailable(oPlugin, oSourceOverlay, oResponsibleElementOverlay) {
				...
				assert.strictEqual(aMenuItems[0].enabled([oSourceOverlay], { action: aMenuItems[0] }), true, "then the menu item is enabled");
				assert.strictEqual(typeof aMenuItems[0].handler, "function", "then menu item handler is a function");
				assert.strictEqual(aMenuItems[0].text, sExpectedText, "then menu item has the expected text");
			...
			sandbox.stub(this.oAddIFrame, "isAvailable").callsFake((aElementOverlays, oAction, bSibling) => {
				if (bSibling === true && (aElementOverlays[0] === this.oButtonOverlay) || aElementOverlays[0] === this.oObjectPageSectionOverlay) {
					return bAvailable;
				}
			});
		...
		QUnit.test("when an overlay has an addIFrame action in designTimeMetadata, and isEnabled property is a function", async function(assert) {
			...
			assert.strictEqual(
				this.oAddIFrame.isEnabled([this.oObjectPageLayoutOverlay], { action: oAction }), true,
			...
			sandbox.stub(this.oAddIFrame, "isEnabled").callsFake(function(aElementOverlays) {
				assert.strictEqual(aElementOverlays[0].getId(), oCheckOverlay.getId(), "then isEnabled() was called with the correct overlay");
			});
