sap.ui.define([
	...
	function (
		...
		const quickCreateDialog = new Dialog({
			title: "Create Item",
			type: "Message",
			content: [
		...
		function onFixedItemPress(event) {
			...
			if (item && item.getKey && item.getKey() === "quickCreate") {
				quickCreateDialog.open();
			}
		}

		model.setData(data);

		const flexibleNavigationList = new NavigationList("NList", {
			items: {
				path: '/navigation',
				factory: function (id, context) {
					...
					if (itemData.type === "group") {
						return new NavigationListGroup(id, {
							text: '{title}',
							expanded: '{expanded}',
							items: {
								path: 'items',
								templateShareable: true,
								template: createNavigationListItem()
							}
						});
					}

					return createNavigationListItem();
				}
			}
		});

		function createNavigationListItem() {
		...
		const sideNavigationSearchField = new SideNavigationSearchField({
			ariaControls: "SNav",
			...
			liveChange: function (event) {
				const value = event.getSource().getValue();
				executeSearch(value);
				flexibleNavigationList.setHighlightedText(value);
			}
		});
		...
		const sideNavigation = new SideNavigation("SNav", {
			...
			filterSection: sideNavigationSearchField,
			item: flexibleNavigationList,
		...
		function countItems(items, value) {
			...
			return items.reduce((count, item) => {
				...
				return count + (titleMatches ? 1 : 0) + countItems(item.items, value);
			}, 0);
		}
				...
				new Button('menuToggleButton', {
					...
					press: function () {
						...
						if (!sideExpanded) {
							...
							sideNavigationSearchField.setValue('');
							executeSearch('');
							flexibleNavigationList.setHighlightedText('');
						}
