...
export default class Generator extends EventEmitter {
	...
	generateLocaleFile(sUI5Tag) {
		...
		Generator.addDatePlaceholderText(oResult, sUI5Tag);
		Generator.addFileSizeTexts(oResult, sUI5Tag);
	...
	/**
	 * Adds "sap-datePlaceholder" text from i18n resource bundle to the given result object for the given language tag.
	 *
	 * @param {object} oResult
	 *   The object of the generator's result into which the translated text is written
	 * @param {string} sUI5Tag
	 *   The UI5 language tag, e.g. "en", "de_CH", "zh_CN" or "sr_Latn"
	 * @throws {Error}
	 *   If the date placeholder text is missing in the i18n resource bundle
	 */
	static addDatePlaceholderText(oResult, sUI5Tag) {
		const oResourceBundle = Generator.getResourceBundle(sUI5Tag);

		oResult["sap-datePlaceholder"] = oResourceBundle?.["date.placeholder"]?.text;
		if (!oResult["sap-datePlaceholder"]) {
			throw new Error("Missing date placeholder texts for " + sUI5Tag);
		}
	}

	/**
	 * Adds "sap-fileSize" texts from i18n resource bundle to the given result object.
	 *
	 * @param {object} oResult
	 *   The object of the generator's result into which the translated text is written
	 * @param {string} sUI5Tag
	 *   The UI5 language tag, e.g. "en", "de_CH", "zh_CN" or "sr_Latn"
	 * @throws {Error}
	 *   If the file size texts are missing in the i18n resource bundle
	 */
	static addFileSizeTexts(oResult, sUI5Tag) {
		const oResourceBundle = Generator.getResourceBundle(sUI5Tag);
		const oFileSize = oResourceBundle?.["FileSize"];
		if (!oFileSize) {
			throw new Error("Missing file size texts for " + sUI5Tag);
		}
		oResult["sap-fileSize"] = {};
		for (const sUnit of Object.keys(oFileSize)) {
			oResult["sap-fileSize"][sUnit] = oFileSize[sUnit]?.text;
			if (!oResult["sap-fileSize"][sUnit]) {
				throw new Error("Missing file size texts for " + sUnit + " in " + sUI5Tag);
			}
		}
	}
