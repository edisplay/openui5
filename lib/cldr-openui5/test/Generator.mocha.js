...
describe("Generator.js", function () {
	...
	[
		...
		it(`generateLocaleFile for locale: ${sUI5Tag}`, function () {
			...
			oGeneratorMock.expects("addCalendarWeekTexts")
				.withExactArgs(sinon.match((oResult0) => oResult0 === oResult), sUI5Tag);
			oGeneratorMock.expects("addDatePlaceholderText")
				.withExactArgs(sinon.match((oResult0) => oResult0 === oResult), sUI5Tag);
			oGeneratorMock.expects("addFileSizeTexts")
				.withExactArgs(sinon.match((oResult0) => oResult0 === oResult), sUI5Tag);
	...
	//*********************************************************************************************
	it("addDatePlaceholderText: success", function () {
		const oResult = {};
		sinon.mock(Generator).expects("getResourceBundle")
			.withExactArgs("~sUI5Tag")
			.returns({"date.placeholder": {text: "~placeholder"}});

		// code under test
		Generator.addDatePlaceholderText(oResult, "~sUI5Tag");

		assert.deepEqual(oResult, {"sap-datePlaceholder": "~placeholder"});
	});
	...
	[
		undefined,
		{},
		{"date.placeholder": {text: undefined}},
		{"date.placeholder": {text: ""}}
	].forEach((oResourceBundle) => {
		it("addDatePlaceholderText: error, " + JSON.stringify(oResourceBundle), function () {
			sinon.mock(Generator).expects("getResourceBundle").withExactArgs("~sUI5Tag")
				.returns(oResourceBundle);

			assert.throws(() => {
				// code under test
				Generator.addDatePlaceholderText({}, "~sUI5Tag");
			}, new Error("Missing date placeholder texts for ~sUI5Tag"));
		});
	});

	//*********************************************************************************************
	it("addFileSizeTexts: success", function () {
		const oResult = {};
		sinon.mock(Generator).expects("getResourceBundle")
			.withExactArgs("~sUI5Tag")
			.returns({"FileSize": {Byte: {text: "~byte"}, Kilobyte: {text: "~kb"}}});

		// code under test
		Generator.addFileSizeTexts(oResult, "~sUI5Tag");

		assert.deepEqual(oResult, {"sap-fileSize": {Byte: "~byte", Kilobyte: "~kb"}});
	});
	...
	[
		...
		{},
		{"FileSize": undefined},
		{"FileSize": {Byte: {text: undefined}}}
	].forEach((oResourceBundle) => {
		it("addFileSizeTexts: error, " + JSON.stringify(oResourceBundle), function () {
			sinon.mock(Generator).expects("getResourceBundle").withExactArgs("~sUI5Tag").returns(oResourceBundle);

			assert.throws(() => {
				// code under test
				Generator.addFileSizeTexts({}, "~sUI5Tag");
			}, /Missing file size texts/);
		});
	});
