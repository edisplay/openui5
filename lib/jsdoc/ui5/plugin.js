...
const {isSemVer, extractSince} = require(`${templatePath}/utils/versionUtil`);
...
function createAutoDoc(oClassInfo, classComment, doclet, node, parser, filename, commentAlreadyProcessed) {
	...
	function newJSDoc(lines) {
		...
		if (doclet?.__ui5?.moduleOverride) {
			const [moduleName, exportName] = doclet.__ui5.moduleOverride;
			lines.push(`@ui5-module-override ${moduleName} ${exportName}`.trim());
		}
...
//	}
	...
	for (const n in oClassInfo.events ) {
		...
		for (const pName in info.parameters ) {
			const optional = info.parameters[pName].optional;
			lines.push(
				"@param {" + (info.parameters[pName].type || "") + "} "
					+ (optional ? "[" : "") + "oControlEvent.getParameters." + pName +  (optional ? "]" : "")
	...
	dictionary.defineTag("since", {
		...
		onTagged: function(doclet, tag) {
			if (isSemVer(doclet.experimental)) {
				warning(`@since and @ui5-experimental-since must not be used both for the same API (${doclet.meta?.lineno})`);
			}
	...
	dictionary.defineTag('ui5-experimental-since', {
		...
		onTagged: function(doclet, tag) {
			if (doclet.since || doclet.experimental) {
				warning(`@experimental/@since and @ui5-experimental-since must not be used together for the same API (${doclet.meta?.lineno})`);
			}
...
const inlineTags = {
	"ui5-experimental-since": {
		onTagged(doclet, symbol, tag) {
			if (symbol.since || symbol.experimental) {
				error(`@experimental/@since and @ui5-experimental-since must not be used together for the same API entity (line:${doclet.meta?.lineno})`);
			}
			const version = tag.value?.trim() ?? "";
			if (!isSemVer(version)) {
				error(`@ui5-experimental-since must have a version and only a version, not '${version}' (line:${doclet.meta?.lineno})`);
			}
			symbol.experimental = version;
		}
	},
	"since": {
		onTagged(doclet, symbol, tag) {
			if (isSemVer(symbol.experimental)) {
				error(`@since and @ui5-experimental-since must not be used both for the same API (line:${doclet.meta?.lineno})`);
			}
			if (!isSemVer(tag.value)) {
				error(`@since must have a version and only a version, not '${tag.value}' (line:${doclet.meta?.lineno})`);
			}
			symbol.since = tag.value;
		}
	},
	"deprecated": {
		onTagged(doclet, symbol, tag) {
			const info = extractSince(tag.value);
			if (!info?.since) {
				error(`@deprecated inline tag must include a version (line:${doclet.meta?.lineno})`);
			}
			if (!info?.value) {
				error(`@deprecated inline tag must include a description (line:${doclet.meta?.lineno})`);
			}
			symbol.deprecated = tag.value; // raw string, parsed again in transformApiJson
		}
	}
};

function fixAndRemoveTagInfo(str, tagInfo) {
	if (tagInfo.text.includes("{@")) {
		const startTag = str.indexOf(tagInfo.completeTag);
		if (startTag === -1) {
			return str.replace(tagInfo.completeTag, "");
		}
		const startText = str.indexOf(tagInfo.text, startTag);
		let openBraces = 1;
		let endTag = startText;
		while (endTag < str.length) {
			const c = str.charAt(endTag++);
			if (c === "{") {
				openBraces++;
			} else if (c === "}") {
				openBraces--;
		...
		const endText = openBraces === 0 ? endTag - 1 : endTag;
		tagInfo.completeTag = str.slice(startTag, endTag);
		tagInfo.text = str.slice(startText, endText).trim();
	}
	return str.replace(tagInfo.completeTag, "");
}

const inlineTagReplacers = Object.keys(inlineTags).reduce(
	(result, key) => {
		result[key] = fixAndRemoveTagInfo;
		return result;
	}, {});
...
exports.handlers = {
	...
	*/
	...
	newDoclet: function(e) {
		...
		// process UI5 inline tags in param tags
		e.doclet.params?.forEach((param) => {
			if (param.description) {
				const result = inline.replaceInlineTags(param.description, inlineTagReplacers);
				if (result.tags.length > 0) {
					param.description = result.newString;
					result.tags.forEach((tagInfo) => {
						const tag = {
							title: tagInfo.tag,
							value: tagInfo.text
						};
						inlineTags[tagInfo.tag].onTagged(e.doclet, param, tag);
					});
				}
			}
		});

		// process UI5 inline tags in @property tags (typedef)
		e.doclet.properties?.forEach((prop) => {
			if (prop.description) {
				const result = inline.replaceInlineTags(prop.description, inlineTagReplacers);
				if (result.tags.length > 0) {
					prop.description = result.newString;
					result.tags.forEach((tagInfo) => {
						const tag = {
							title: tagInfo.tag,
							value: tagInfo.text
						};
						inlineTags[tagInfo.tag].onTagged(e.doclet, prop, tag);
					});
				}
			}
		});
...
(function() {
	...
	jsdocType.parse = function() {
		...
		// #### BEGIN: MODIFIED BY SAP
		if (tagInfo && tagInfo.name && /^\[|=/.test(tagInfo.name)) {
			// debug("tag info before post-processing the name-portion", tagInfo);
			...
			if (p > 0) { // intentionaly checks for != 0 (no empty name allowed)
				tagInfo.defaultvalue = name.slice(p + 1).trim();
				name = name.slice(0, p).trim();
			}
			tagInfo.name = name;
			// debug("tag info after post-processing the name-portion", tagInfo);
		}
		// #### END: MODIFIED BY SAP
		return tagInfo;
	};
