...
const {extractSince} = require("./ui5/template/utils/versionUtil");
const log = (function() {
	try {
...
function transformer(sInputFile, sOutputFile, sLibraryFile, vDependencyAPIFiles, sSectionsDir, options) {
		...
		oData.symbols.forEach((oSymbol) => {
			...
			if (oSymbol.constructor) {
				...
				if (oConstructor.parameters) {
					...
					aParameters.forEach((oParameter) => {
						...
						// Experimental
						if (oParameter.experimental) {
							oParameter.experimentalText = formatters.formatExperimental(oParameter.experimental.since ?? oParameter.experimental);
						}

						// Deprecated
						if (oParameter.deprecated) {
							const dep = typeof oParameter.deprecated === "string"
								? extractSince(oParameter.deprecated)
								: { since: oParameter.deprecated.since, value: oParameter.deprecated.text };
							oParameter.deprecatedText = formatters.formatDeprecated(dep?.since, dep?.value);
						}
			...
			if (oSymbol.properties) {
				...
				oSymbol.properties.forEach((oProperty) => {
					...
					// Experimental
					if (oProperty.experimental) {
						oProperty.experimentalText = formatters.formatExperimental(oProperty.experimental.since ?? oProperty.experimental);
					}

					// Deprecated
					if (oProperty.deprecated) {
						const dep = typeof oProperty.deprecated === "string"
							? extractSince(oProperty.deprecated)
							: { since: oProperty.deprecated.since, value: oProperty.deprecated.text };
						oProperty.deprecatedText = formatters.formatDeprecated(dep?.since, dep?.value);
					}
			...
			if (oSymbol["ui5-metadata"]) {
				...
				if (oMeta.aggregations) {
					...
					oMeta.aggregations.forEach((oAggregation) => {
						...
						// Experimental
						if (oAggregation.experimental) {
							oAggregation.experimentalText = formatters.formatExperimental(oAggregation.experimental.since ?? oAggregation.experimental);
						}
				...
				if (oMeta.associations) {
					...
					oMeta.associations.forEach((oAssociation) => {
						...
						// Experimental
						if (oAssociation.experimental) {
							oAssociation.experimentalText = formatters.formatExperimental(oAssociation.experimental.since ?? oAssociation.experimental);
						}

						// Deprecated
						if (oAssociation.deprecated) {
							oAssociation.deprecatedText = formatters.formatDeprecated(oAssociation.deprecated.since,
								oAssociation.deprecated.text);
						}
					});
				}
			...
			if (oSymbol.methods) {
				...
				oSymbol.methods.forEach((oMethod) => {
					...
					if (oMethod.deprecated) {
						oMethod.deprecatedText = formatters.formatMethodDeprecated(oMethod.deprecated.since,
	...
	const formatters = {
		...
		formatExperimental: function (sExperimentalSince) {
			return `Experimental since ${sExperimentalSince}`;
		},
